package io.github.nopeless;


import io.github.nopeless.player.AlwaysRejectPlayer;
import io.github.nopeless.player.BasicStrategyPlayer;
import io.github.nopeless.player.RandomPlayer;
...
//        game.addPlayer(new AlwaysRejectPlayer());
        game.addPlayer(new RandomPlayer());
        game.addPlayer(new RandomPlayer());
        game.addPlayer(new RandomPlayer());
        game.addPlayer(new BasicStrategyPlayer());
...
//        playOneGame(game);          // simulate one game...
    ...
    private static void playManyGames(NoThanksGame game) {
        ...
        for (int x = 0; x < numgames; x++) {
            game.resetGame();
            game.playGame();
            System.out.println("Game " + x + " final scores: " + game.getFinalScores() + " " + game.getFinalChips()
